...
int main (void){
	...
	double metropolis_avg = 0.;
	double exact_avg = 0.;
	for (int i = 0; i< num_energies; i++){
		metropolis_avg += energy_i(i)*dist_metropolis[i];
		exact_avg += energy_i(i)*dist_exact[i];
	}
	cout << endl << "Exact average: "<< exact_avg 
		 << " Metropolis average: " << metropolis_avg
		 << "\n\n";

	//*******************************************************************
	// output the distributions of energies P(E)
	ofstream out;
	out.open("sampling_test_kT1.dat");
	cout << "#  energy     exact      random      metropolis  " << endl;
	out << "#  energy     exact      random      metropolis  " << endl;
...
//
//*************************************************************************
double calculate_energy (int configuration[]){
	int nearest = 0;
	double energy = 0.;
...
//
//*************************************************************************
int next_configuration (int configuration[], int k){
